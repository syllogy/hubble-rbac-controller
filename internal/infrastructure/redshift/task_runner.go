...
func (t *TaskRunnerImpl) CreateUser(model *redshift.UserModel) error {
	...
	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).CreateUser(model.User.Name)
...
func (t *TaskRunnerImpl) DropUser(model *redshift.UserModel) error {
	...
	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).DeleteUser(model.User.Name)
...
func (t *TaskRunnerImpl) CreateGroup(model *redshift.GroupModel) error {
	...
	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).CreateGroup(model.Group.Name)
...
func (t *TaskRunnerImpl) DropGroup(model *redshift.GroupModel) error {
	...
	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).DeleteGroup(model.Group.Name)
	if err != nil {
...
func (t *TaskRunnerImpl) CreateSchema(model *redshift.SchemaModel) error {
	...
	err :=  t.clientPool.GetDatabaseClient(model.Database.ClusterIdentifier, model.Database.Name).CreateSchema(model.Schema.Name)
	if err != nil {
...
func (t *TaskRunnerImpl) CreateExternalSchema(model *redshift.ExternalSchemaModel) error {
	...
	err := t.clientPool.GetDatabaseClient(model.Database.ClusterIdentifier, model.Database.Name).CreateExternalSchema(model.Schema.Name, model.Schema.GlueDatabaseName, t.awsAccountId)
...
func (t *TaskRunnerImpl) CreateDatabase(model *redshift.DatabaseModel) error {
	...
	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).CreateDatabase(model.Database.Name, model.Database.Owner)
...
func (t *TaskRunnerImpl) GrantAccess(model *redshift.ManageAccessModel) error {
	...
	err := t.clientPool.GetDatabaseClient(model.Database.ClusterIdentifier, model.Database.Name).Grant(model.GroupName, model.SchemaName)
...
func (t *TaskRunnerImpl) RevokeAccess(model *redshift.ManageAccessModel) error {
	...
	err := t.clientPool.GetDatabaseClient(model.Database.ClusterIdentifier, model.Database.Name).Revoke(model.GroupName, model.SchemaName)
...
func (t *TaskRunnerImpl) AddToGroup(model *redshift.ManageMembershipModel) error {
	t.log.Info(fmt.Sprintf("AddToGroup (%s) %s->%s", model.ClusterIdentifier, model.Username, model.GroupName))

	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).AddUserToGroup(model.Username, model.GroupName)
	if err != nil {
		return fmt.Errorf("unable to add user %s to group %s in %s: %w", model.Username, model.GroupName, model.ClusterIdentifier, err)
	}
	return nil
}
func (t *TaskRunnerImpl) RemoveFromGroup(model *redshift.ManageMembershipModel) error {
	t.log.Info(fmt.Sprintf("RemoveFromGroup (%s) %s->%s", model.ClusterIdentifier, model.Username, model.GroupName))

	err := t.clientPool.GetClusterClient(model.ClusterIdentifier).RemoveUserFromGroup(model.Username, model.GroupName)
	if err != nil {
