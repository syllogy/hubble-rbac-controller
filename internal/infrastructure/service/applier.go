...
func (applier *Applier) Apply(model hubble.Model, dryRun bool) error {
	...
	applier.logger.Info("Received hubble model", "model", applier.toString(model))
	...
	applier.logger.Info("Applying redshift model", "model", applier.toString(resolved.RedshiftModel))
	if !dryRun {
	...
	applier.logger.Info("Applying IAM model", "model", applier.toString(resolved.IamModel))
	if !dryRun {
	...
	applier.logger.Info("Applying Google model", "model", applier.toString(resolved.GoogleModel))
	if !dryRun {
